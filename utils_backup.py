...
def paired_edge_centerlines(binary_mask, axis_label, min_fraction=0.45, expected_n_dividers=None, large_gap_cutoff=None):
    """
    ...
    ###############
    print(f"{axis_label.upper()} min_fraction:")
    print(min_fraction)
    ###############
    ...
    ###############
    print()
    print(f"{axis_label.upper()} occupancy range:")
    print(f"{occupancy.min():.3f} to {occupancy.max():.3f}")

    print(f"{axis_label.upper()} n_candidates:")
    print(len(candidate_idxs))

    print(f"{axis_label.upper()} candidate_idxs:")
    print(candidate_idxs)
    ...
    plt.axhline(
        ...
        color="red",
        linestyle="--",
        label=f"threshold = {min_fraction:.3f}"
    )

    plt.legend()
    plt.title(f"{axis_label.upper()} occupancy profile")
    plt.xlabel(f"{axis_label.lower()} index")
    plt.ylabel("occupancy")
    plt.show()
    ###############

    candidate_neighborhoods = partition_candidate_neighborhoods(
        candidate_idxs,
        expected_n_dividers=expected_n_dividers,
        large_gap_cutoff=large_gap_cutoff,
        max_value=len(occupancy),
    )
    ...
    if len(candidate_idxs) > 1:
        print()
        print(f"{axis_label.upper()} candidate gaps:")
        print(np.diff(candidate_idxs))

    print()
    print(f"{axis_label.upper()} candidate_neighborhoods:")
    for i, neighborhood in enumerate(candidate_neighborhoods, start=1):
        print(f"  neighborhood {i}: {neighborhood}")

    ###############
    print(f"{axis_label.upper()} candidate_neighborhoods:")
    print(candidate_neighborhoods)
    ###############
    ...
    for neighborhood in candidate_neighborhoods:
        ...
        neighborhood_bands, neighborhood_band_centers = split_neighborhood_into_edge_sets(
            neighborhood,
            max_value=len(occupancy)
        )

        print()
        print(f"{axis_label.upper()} neighborhood:")
        print(neighborhood)

        print(f"{axis_label.upper()} neighborhood_bands:")
        print(neighborhood_bands)

        print(f"{axis_label.upper()} neighborhood_band_centers:")
        print(neighborhood_band_centers)

        candidate_bands.extend(neighborhood_bands)
        band_centers.extend(neighborhood_band_centers.tolist())

        if len(neighborhood_band_centers) < 2:
            continue

        idx = float(neighborhood_band_centers[0])
        partner = float(neighborhood_band_centers[1])

        pairs.append([idx, partner])
        centerlines.append(int(round((idx + partner) / 2)))

        print()
        print(f"{axis_label.upper()} pairing result")
        print(
            f"  accepted pair: "
            f"{idx} <-> {partner} "
            f"centerline={int(round((idx + partner) / 2))}"
        )

    band_centers = np.array(band_centers)

    ###############
    print(f"{axis_label.upper()} candidate_bands:")
    print(candidate_bands)

    print(f"{axis_label.upper()} band_centers:")
    print(band_centers)
    ###############
...
#        used.add(partner)
    ...
    ###############
    print(f"{axis_label.upper()} pairs:")
    print(pairs)

    print(f"{axis_label.upper()} centerlines:")
    print(centerlines)
    ###############
