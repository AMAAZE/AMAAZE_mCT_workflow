...
This report was generated on: **June 10, 2026 at 12:16 PM**
...
|---|---|
| Dataset name: **bonefrags** | Specimens identified: **8** |
| Scan number: **2** | Specimens extracted: **8**|
| Number of input slices: **3439** | Surface meshes generated: **8** |
| Layout file: **layout_bone_data_cropped.csv** | Cleaned meshes produced: **8** |
...
| Setting | Value |
|---|---|
| In-plane voxel size | **0.046 mm** |
| Voxel (Slice) spacing | **0.0046 mm** |
| Slice Index Fraction | **0.18** |
| Rotation angle | **-90.0 degrees** |
| Transpose preview applied | **True** |
| Crop row range | **[0, 58]** |
| Crop column range | **[1, 163]** |
| z-window | **1** |
| ISO value | **15000.0** |
...
| Expected tiers | **2** |
| Active tiers | **2** |
| Tier detection method | **automatic_peaks** |
| Tier order reversed | **True** |
| Active tier ranges | **[[1658, 3439], [59, 1658]]** |
| Expected specimens | **8** |
| Specimens identified | **8** |
| Extraction regions generated | **8** |
...
| Setting | Value |
|---|---|
| Specimens extracted | **8** |
| Meshes generated | **8** |
| Surfacing errors | **0** |

### Output folder

**I:\AISOS_Users\AISOS_AMAAZE_mCT_workspace\AMAAZE_mCT_workflow\test_data_bonefrags_CT_2\bonefrags_scan2_AMAAZE_outputs** 
...
|---|---:|---:|---:|
| 00. Data intake and setup | 2.1 min | — | 2.1 min |
| 01. Orientation and cropping | 91.0 sec | — | 91.0 sec |
| 02. Volume construction | — | 29.8 sec | 29.8 sec |
| 03. Segmentation and extraction planning | 2.5 min | — | 2.5 min |
| 04. Specimen extraction and surfacing | 26.3 sec | 38.1 min | 38.5 min |
| 05. Mesh cleaning | 33.0 sec | 1.6 sec | 34.7 sec |
| **Workflow total** | **7.0 min** | **38.6 min** | **45.7 min** |
...
**I:\AISOS_Users\AISOS_AMAAZE_mCT_workspace\AMAAZE_mCT_workflow\test_data_bonefrags_CT_2\bonefrags_scan2_AMAAZE_outputs**
- Metadata: 
**I:\AISOS_Users\AISOS_AMAAZE_mCT_workspace\AMAAZE_mCT_workflow\test_data_bonefrags_CT_2\bonefrags_scan2_AMAAZE_outputs\bonefrags_scan2_metadata.json**
- Scan folder: 
**I:\AISOS_Users\AISOS_AMAAZE_mCT_workspace\AMAAZE_mCT_workflow\test_data_bonefrags_CT_2**
- Slice folder: 
**I:\AISOS_Users\AISOS_AMAAZE_mCT_workspace\AMAAZE_mCT_workflow\test_data_bonefrags_CT_2\Slices_cropped**
- Layout CSV: 
**I:\AISOS_Users\AISOS_AMAAZE_mCT_workspace\AMAAZE_mCT_workflow\test_data_bonefrags_CT_2\layout_bone_data_cropped.csv**
...
The representative slice selected using the slice index fraction (**0.18**) from the previous step was displayed for user review. The slice index fraction defines the relative position of the preview image within the scan volume.

The user compared the scan orientation against the specimen layout and determined whether the image axes should be transposed. For this run, transpose preview was set to **True**, indicating whether the image rows and columns were swapped before rotation and cropping.

After orientation review, the scan was rotated to align the package and specimen grid with the image axes. Rotation was performed interactively and visually confirmed by the user. The accepted rotation angle (**-90.0 degrees**) was applied to align the package and specimen grid with the image axes.

Following orientation correction, the user defined a crop region encompassing the area of interest. This crop removed portions of the image that were not relevant to specimen extraction and reduced computational requirements for later processing steps. The accepted crop retained **rows [0, 58]** and **columns [1, 163]**, defining the region that was carried forward into all subsequent workflow stages.
...
For this run, the reduced working volume had dimensions **[3439, 81, 225]**.
...
In Step 03, the reduced volume was used to identify package tiers, detect specimen boundaries, and construct the specimen extraction plan. The workflow began by loading the reduced working volume created during Step 02 and the specimen layout CSV (**layout_bone_data_cropped.csv**) supplied during data intake. The layout file defines the expected organization of specimens within the package and provides the specimen identifiers used throughout the remainder of the workflow.
...
**[{'tier_id': 1, 'rotation_angle': 0.0, 'angle_min': -5, 'angle_max': 5, 'angle_step': 0.25, 'best_score': 0.7206594886091096}, {'tier_id': 2, 'rotation_angle': 0.0, 'angle_min': -5, 'angle_max': 5, 'angle_step': 0.25, 'best_score': 0.6430609397528838}]**
...
**[{'tier_id': 1, 'proposed_row_dividers': [51], 'proposed_col_dividers': [71, 146], 'final_row_dividers': [51], 'final_col_dividers': [71, 146], 'divider_method': 'automatic_accepted'}, {'tier_id': 2, 'proposed_row_dividers': [45], 'proposed_col_dividers': [77, 152], 'final_row_dividers': [45], 'final_col_dividers': [77, 152], 'divider_method': 'automatic_accepted'}]**

### Specimen extraction regions

After tier boundaries and divider locations were finalized, the workflow generated extraction regions corresponding to the specimen positions defined in the layout file and saved the information to an extraction plan. The resulting extraction plan (**I:\AISOS_Users\AISOS_AMAAZE_mCT_workspace\AMAAZE_mCT_workflow\test_data_bonefrags_CT_2\bonefrags_scan2_AMAAZE_outputs\bonefrags_scan2_extraction_plan.csv**) defines the spatial boundaries used during Step 04 to reconstruct specimen-specific subvolumes from the original full-resolution scan data.
...
The extraction plan generated during Step 03 was used to reconstruct individual specimen volumes from the original scan. A total of **8** volumes were extracted.

### Surface generation

After extraction, each specimen volume was converted into a surface mesh using the selected isovalue. During this run, **8** meshes were successfully generated and **0** were recorded. If any errors occurred, the surfacing error log contains the affected specimen identifiers and diagnostic information that may assist with troubleshooting. The surfacing error log was saved as:

**I:\AISOS_Users\AISOS_AMAAZE_mCT_workspace\AMAAZE_mCT_workflow\test_data_bonefrags_CT_2\bonefrags_scan2_AMAAZE_outputs\surfacing_errors.csv**
...
The workflow evaluated **8** meshes generated during Step 04 and applied the mesh-cleaning procedure to each specimen individually. During this run, **8** meshes were successfully cleaned and **0** mesh-cleaning failures were recorded.

Mesh-cleaning outcomes are summarized below:

**0 mesh-cleaning failures recorded. See mesh-cleaning log: I:\AISOS_Users\AISOS_AMAAZE_mCT_workspace\AMAAZE_mCT_workflow\test_data_bonefrags_CT_2\bonefrags_scan2_AMAAZE_outputs\mesh_cleaning_log.csv**

The mesh-cleaning log was saved as:

**I:\AISOS_Users\AISOS_AMAAZE_mCT_workspace\AMAAZE_mCT_workflow\test_data_bonefrags_CT_2\bonefrags_scan2_AMAAZE_outputs\mesh_cleaning_log.csv**
...
**I:\AISOS_Users\AISOS_AMAAZE_mCT_workspace\AMAAZE_mCT_workflow\test_data_bonefrags_CT_2\bonefrags_scan2_AMAAZE_outputs\Clean_Meshes**
