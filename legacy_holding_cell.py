...
# TEMPORARILY DISABLED DURING METADATA REFACTOR
# ============================================================


############### Stuff below here scootched down for a moment while we create 00 and refactor 01. 
# We can delete what we end up not needing, change what needs changing, then organize into appropriate categories above.
# ============================================================
...
try:
    ...
    reverse_detected_tier_order = CONFIG["reverse_detected_tier_order"]
...
def select_tier_boundaries_from_signal(q, candidate_peaks, n_tiers, peak_band_gap=100):
    expected_n_boundaries = n_tiers + 1

    candidate_peaks = np.sort(np.array(candidate_peaks).astype(int))

    # Always include scan start/end as boundaries.
    internal_peaks = candidate_peaks[
        (candidate_peaks > 0) & (candidate_peaks < len(q))
    ]

    bands, _ = collapse_candidate_bands(
        internal_peaks,
        max_band_gap=peak_band_gap
    )
    ...
    for band in bands:
        ...
        best_positions = band[values == max_value]
        selected_internal.append(int(round(best_positions.mean())))

    selected_internal = np.array(selected_internal).astype(int)

    # Keep strongest internal boundaries if too many were found.
    if len(selected_internal) > n_tiers - 1:
        strengths = q[selected_internal]
        keep = np.argsort(strengths)[-(n_tiers - 1):]
        selected_internal = np.sort(selected_internal[keep])
